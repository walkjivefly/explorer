...
function update_address(hash, txid, amount, type, cb) {
  ...
  find_address(hash, function(address) {
    if (address) {
      ...
      if ( hash == 'coinbase' || hash == 'coinstake' ) {
...
function save_block(block, cb) {
  var newBlock = new Block({
    ...
    height: block.height,
    type: block.proof_type,
    timestamp: block.time,
    hashprev: block.previousblockhash,
    vtx: block.tx,
...
module.exports = {
  ...
  get_last_blocks: function(count, min, cb) {
    Block.find({'height': {$gt: min}}).sort({_id: 'desc'}).limit(count).exec(function(err, txs){
      if (err) {
        return cb(err);
      } else {
        return cb(txs);
        ...
        if (typeof(blockhash) !== 'undefined') {
          lib.get_block(blockhash, function(block) {
            if (block) {
              ...
              Block.findOne({blockhash: blockhash}, function(err, blocknew) {
                if (blocknew) {
                  blocknew = null;
                } else {
                  save_block(block, function(err){
                    if (err) {
                      console.log(err);
                    } else {
                      console.log('Saved block %s: %s', block.height, blockhash);
                    }
                    setTimeout( function(){
                      blocknew = null;
                    }, timeout);
