...
module.exports = {
  ...
  get_input_addresses: function(input, vout, cb) {
    var addresses = [];
    if (input.coinbase || input.coinstake) {
      var amount = 0;
      module.exports.syncLoop(vout.length, function (loop) {
        var i = loop.iteration();
        ...
        if (input.coinbase)
          ...
          addresses.push({hash: 'coinstake', amount: amount});
        return cb(addresses);
      });
      ...
      var out_index = 0;
      if (input.coinstake) {
        txid = input.pointer_hash;
        out_index = input.pointer_n;
      } else {
        txid = input.txid;
        out_index = input.vout;
      }
      module.exports.get_rawtransaction(txid, function(tx){
        if (tx) {
          module.exports.syncLoop(tx.vout.length, function (loop) {
            var i = loop.iteration();
            if (tx.vout[i].n == out_index) {
